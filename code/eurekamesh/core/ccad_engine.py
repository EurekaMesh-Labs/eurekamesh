...
class CCAdEngine:
    ...
    def __init__(
        ...
        fuzzy_threshold: float = 0.95,  # For near-duplicate detection
    ...
    def build_anti_dup_context(self, use_rag: bool = None) -> str:
        ...
        if use_rag and self.retrieval:
            ...
            try:
                ...
                try:
                    ...
                    if not prototypes:
                        prototypes = list(self.seen_canonical)[-self.max_context_items:]
        ...
        if not use_rag:
            ...
            recent = list(self.seen_canonical)[-self.max_context_items:]
            context = (
                f"\n\n⚠️  CRITICAL ANTI-DUPLICATION REQUIREMENT:\n"
                f"You have ALREADY generated {len(self.seen_canonical)} unique items.\n\n"
    ...
    async def process_batch(
        ...
        for candidate in candidates:
            ...
            # 5. (Optional) Fuzzy deduplication - ADVISORY ONLY (optimized RAG)
            # Changed: Don't reject, just log for guidance in future iterations
            if enable_fuzzy and self.distance_metric:
                max_similarity = 0.0
                # Only check against recent items for performance
                recent_objects = self.seen_objects[-min(100, len(self.seen_objects)):]
                
                try:
                    for seen_obj in recent_objects:
                        dist = self.distance_metric.distance(candidate, seen_obj)
                        similarity = 1.0 - dist
                        max_similarity = max(max_similarity, similarity)
                        
                        # If very similar, log but DON'T reject
                        if similarity > self.fuzzy_threshold:
                            self.logger.debug(
                                f"High similarity ({similarity:.2f}) detected, "
                                f"but accepting (advisory fuzzy dedup)"
                            )
                            # NOTE: Not incrementing near_dups, not continuing
                            # This allows candidate to proceed
                except Exception as e:
        ...
        # Chunk metrics
        # IMPORTANT: For UPT calculation, only count VALID candidates (exclude invalid SMILES)
        # This matches the original implementation's behavior
        valid_generated = len(unique_accepted) + exact_dups + near_dups
        
        chunk_metrics = {
            'generated': valid_generated,  # Only valid candidates (matches original)
            'unique_accepted': len(unique_accepted),
            'exact_duplicates': exact_dups,
            'near_duplicates': near_dups,
            'invalid': invalid,
            'duplicate_rate': (exact_dups + near_dups) / max(1, valid_generated)
        }
        
        # Update global metrics
        self.metrics.add_chunk(
            generated=valid_generated,  # Only count valid for UPT calculation
            unique=len(unique_accepted),
    ...
    async def generate(
        ...
        for idx, (prompt, target_count) in enumerate(zip(prompts, target_counts)):
            ...
            while remaining > 0 and iteration < max_iterations:
                ...
                unique_items, chunk_metrics = await self.process_batch(raw_items)
