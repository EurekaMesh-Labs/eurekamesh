...
MAX_PER_CALL = int(os.getenv('MAX_PER_CALL', '50'))
MIN_PER_CALL = int(os.getenv('MIN_PER_CALL', '10'))
...
async def run_condition(mode: str, target: int = TARGET) -> Dict[str, Any]:
    ...
    if mode == 'rag':
        try:
            ...
            retrieval = None
            enable_rag = False
    # Optional fuzzy plugin (chemistry)
    distance_metric = None
    if ENABLE_FUZZY:
        try:
            from eurekamesh.domains.drug_discovery.rdkit_distance import RdkitECFPDistance
            distance_metric = RdkitECFPDistance()
        except Exception:
            distance_metric = None
    engine = CCAdEngine(
        canonicalizer=canon,
    ...
    res = await engine.generate(
        ...
        max_per_call=MAX_PER_CALL, min_per_call=MIN_PER_CALL, max_iterations=60,
