...
- Domain adapters expose a minimal interface; the molecules adapter in the repo is just a reference implementation.
- Reproducibility: MOCK_LLM offline mode, deterministic fallback for embeddings, CI and make targets.

## Metrics (6–10 lines)
We track three families of metrics:
- UPT_raw = unique_accepted / total_generated (how many unique items per total tokens produced).
- UPT_valid = unique_accepted / total_valid_generated (when only valid items form the denominator).
- Duplicate rates: dup_rate_raw = duplicates / total_generated; dup_rate_valid = duplicates / total_valid_generated.
- Validity rate = valid / total_generated (domain‑specific validation).
Each run logs JSONL with chunk‑level history; CSV aggregates support figures and HTML reports. Costs are estimated from token usage.
...
repeat until N uniques:
  ...
  if dup_rate > high: increase context K, tighten fuzzy(threshold/policy)
...
- If `dup_rate` rises: increase `K` (anti‑dup context), switch `fuzzy_policy` to `reject` or lower threshold, or vary the prompt.
...
## Domain Adapter Interface (the key piece)
...
The molecules adapter in this repo is only a reference; the same pattern applies to text/code/materials.

## Mini demo (agnostic example)
...
banana  # exact dup
apples  # near‑dup (Levenshtein similarity)
