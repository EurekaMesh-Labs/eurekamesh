...
async def call_llm(prompt: str, temperature: float, n: int) -> (List[str], dict):
    items, usage = await connector.generate(SYSTEM_PROMPT, prompt, MODEL_NAME, temperature, MAX_TOKENS)
    return items, usage
...
async def main():
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["run","total","valid","unique_canonical","upt","valid_rate","prompt_tokens","completion_tokens","cost_usd","timestamp"]) 
        for r in results:
            w.writerow([r['run'], r['total'], r['valid'], r['unique_canonical'], round(r['upt'],4), round(r['valid_rate'],4), r['prompt_tokens'], r['completion_tokens'], round(r['cost_usd'],6), r['timestamp']])
        w.writerow([])
