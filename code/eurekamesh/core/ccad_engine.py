...
class CCAdEngine:
    ...
    async def process_batch(
        ...
        for candidate in candidates:
            ...
            # 5. (Optional) Fuzzy deduplication via DistanceMetric (domain-agnostic)
            is_near_dup = False
            if enable_fuzzy and self.distance_metric is not None:
                try:
                    recent_canon = self.canonical_history[-min(200, len(self.canonical_history)):]
                    if recent_canon:
                        # Distance in [0,1]; convert to similarity
                        dists = self.distance_metric.batch_distance(canonical, recent_canon)
                        if dists:
                            max_sim = max(1.0 - float(d) for d in dists)
